#Adapted directly from https://github.com/christophM/rulefit
...
class RuleFit(BaseEstimator, TransformerMixin):
    ...
    ----------
        ...
        tol:            The tolerance for the optimization for ElasticNetCV or LogisticRegressionCV:
        ...
        max_iter:       The maximum number of iterations for ElasticNetCV or LogisticRegressionCV.
        n_jobs:         Number of CPUs to use during the cross validation in ElasticNetCV or
    ...
    def __init__(
        ...
        self.cv=cv
        self.tol=tol
        # ElasticNetCV default max_iter is 1000 while LogisticRegressionCV 100.
    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if self.rfmode=='regress':
            if self.Cs is None: # use defaultshasattr(self.Cs, "__len__"):
                ...
                n_alphas= self.Cs
                alphas=None
            self.enetcv = ElasticNetCV(
                l1_ratio=l1_ratio,
                n_alphas=n_alphas, alphas=alphas, cv=self.cv,
                max_iter=self.max_iter, tol=self.tol,
                n_jobs=self.n_jobs,
                random_state=self.random_state)
            self.enetcv.fit(X_concat, y)
            self.coef_=self.enetcv.coef_
            self.intercept_=self.enetcv.intercept_
        else:
            Cs=10 if self.Cs is None else self.Cs
            self.enetcv=LogisticRegressionCV(
            ...
            self.enet.fit(X_concat, y)
            self.coef_=self.enet.coef_[0]
            self.intercept_=self.enetcv.intercept_[0]
    ...
    def predict(self, X):
        ...
        if 'r' in self.model_type:
            ...
            if len(rule_coefs)>0:
                ...
                if X_rules.shape[0] >0:
                    X_concat = np.concatenate((X_concat, X_rules), axis=1)
        return self.enetcv.predict(X_concat)
    ...
    def predict_proba(self, X):
        ...
        if 'predict_proba' not in dir(self.enetcv):

            error_message = '''
            Probability prediction using predict_proba not available for
            model type {enetcv}
            '''.format(enetcv=self.enetcv)
        ...
        if 'r' in self.model_type:
            ...
            if len(rule_coefs)>0:
                ...
                if X_rules.shape[0] >0:
                    X_concat = np.concatenate((X_concat, X_rules), axis=1)
        return self.enetcv.predict_proba(X_concat)
